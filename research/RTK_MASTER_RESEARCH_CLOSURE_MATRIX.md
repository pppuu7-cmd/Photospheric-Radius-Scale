...
| A2 | ΛCDM local dense reference / best-known basin | 🟡 RUNNING | Historical exact score `1049.966118347761` remains reproducible but its accepted-score point is not Hessian-certified. The frozen cross-basin continuation has now resolved the discovered line direction: exact `t=1.1` gives `1049.3633203461366`; replay error `2.27e-13`; every frozen forward sample through `t=1.9` is higher, so forward improvement beyond `t=1.1` is `0.0`. Mandatory gate is now the frozen full 6D `t=1.1` base stationarity target `A5_LCDM_T1P1_STATIONARITY_TARGET_v1.json`, not further line extension. |
...
| A5 | Best-known matched raw ΔS | 🟡 RUNNING | Historical exact pair delta `+0.2837940820259064` is retained only as historical bookkeeping. Using certified RTK and current best exact LCDM navigation point `t=1.1` gives navigation-only `ΔS=+0.8865920836503844`. Do not freeze it until the `t=1.1` base decision is resolved, any required rays/recenter are completed, an independent half-scale gate passes where admissible, fresh-tree replay passes, and a common paired replay is frozen and reproduced. |
...
| B8 | Nonlinear/local-gravity/compact-object phenomenology | 🔴 OPEN | Linear static closure cannot by itself generate the desired asymptotic logarithmic galaxy potential; nonlinear isolated-system, solar-system and compact-object/universal-horizon gates remain mandatory. The new same-action static reduction is a prerequisite only, not a local-gravity closure. |
...
| C7 | Full coupled metric + causal RT + Khronon nonlinear DOF/ghost theorem | 🟡 RUNNING | The fixed U(1)+DBI representative now has two exact same-action subgates: `RTK_C8_U1_TT_PRINCIPAL_GATE_RESULT_v1.json` gives pure-TT principal `c_T^2=1` with no `S_mix` contribution, and `RTK_C8_U1_STATIC_SCALAR_REDUCTION_RESULT_v1.json` proves `Sigma=q t` solves the restricted static scalar EOM. The static theorem also shows `S_mix = M_Pl^2 a_i a^i` exactly, equivalent to an order-one `beta0` contribution `2`; therefore pure-U(1) PPN formulae cannot be imported unchanged. Next mandatory gate is a weak-field lapse/spatial-metric/U(1) constraint derivation from this same full fixed action, followed by DOF/ghost closure. |
...
| C9 | Radiative stability / counterterm closure / naturalness | 🔴 OPEN | `n=2` intrinsic-curvature carrier has exact nonlinear kernels, but soft-s channel invalidated the earlier optimistic all-hard UV sufficiency. The exact static reduction exposes an unsuppressed `M_Pl^2 a_i a^i` operator from `S_mix`; its stability under the same-action counterterm/constraint structure must be included rather than treated as a tunably small background artifact. Full mixed-sector unitarity and technical naturalness remain open. |
...
| D5 | Autonomous decision-tree regression coverage | 🚀 MAJOR CLOSED | Workflow success is separated from scientific classification. The `t=1.1` base→half router dispatches half only after a persisted recenter-clear PD base; recenter or non-PD states stop safely. The half target was preregistered before base scores. New U(1) TT/static results each have frozen-before-result symbolic targets and CI replay workflows. |
...
1. **A5:** inspect `A5_LCDM_T1P1_BASE_RESULT_v1.json` when persisted. If improvement `>0.005`, freeze exact recenter before any further scale. If recenter-clear and non-PD, freeze exact eigenmode rays. If recenter-clear and PD, the already-preregistered identical-center half-scale workflow may run automatically.
2. **A5:** if an admissible half-scale result is recenter-clear and PD, require independent fresh-tree replay at the identical center; only then freeze a new common RTK/LCDM paired replay and update `research/state/current.json`.
3. **B4:** finish v4 half eigenmode rays; only a persisted no-descent result may route into the preregistered v4 quarter Hessian. Fresh-tree remains mandatory after a recenter-clear PD quarter.
4. **B5:** finish B5-LIN, then use `B5_SURVEY_AP_NONLINEAR_SCOPE_PROTOCOL_2026-08-24.md` to freeze a separate survey/template adequacy target; do not infer survey closure from linear scale dependence alone.
5. **C7 same-action:** derive the weak-field static lapse/spatial-metric/U(1) constraint system from the fixed U(1)+DBI action with the exact reduction `S_mix=M_Pl^2 a_i a^i` and the quadratic DBI lapse term. Do not import a pure-U(1) PPN result that omitted `S_mix`. Then solve the Newton/PPN sector and continue scalar/vector DOF/ghost analysis on that same action.

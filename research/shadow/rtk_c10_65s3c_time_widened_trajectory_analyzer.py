...
def main():
    ...
    t=L('research/theory_targets/RTK_C10_65S3C_TIME_WIDENED_TRAJECTORY_TARGET_v1.json');s3b=L('research/theory_results/RTK_C10_65S3B_TRAJECTORY_SAMPLED_CONSTRAINT_RESULT_v1.json');s3a=L('research/theory_results/RTK_C10_65S3A_FINITE_SHORT_INTERVAL_ENDPOINT_STABILITY_RESULT_v1.json');k2=L('research/theory_results/RTK_C10_65S2K_ONE_ACCEPTED_STEP_RETRY_RESULT_v1.json')
    assert t['status']=='FROZEN_BEFORE_EXECUTION';assert s3b['classification']=='C10_65S3B_TRAJECTORY_SAMPLED_CONSTRAINT_PASS_SCOPED';assert s3a['classification']=='C10_65S3A_FINITE_SHORT_INTERVAL_ENDPOINT_STABILITY_PASS_SCOPED';assert k2['classification']=='C10_65S2K_ONE_ACCEPTED_STEP_RETRY_PASS_SCOPED'
    ...
    atol=float(t['sampling_policy']['endpoint_width_absolute_tolerance_Mpc']);lim=float(t['constraint_policy']['max_abs_normalized_momentum']);allbefore=True;allwidth=True;allfinite=True;allapprox=True;allcf=True;allc=True;allchg=True;records=[];maxres=0.;maxchg=0.
    for entry in man:
        ...
        for b,z in zip(B,A):
            dw=z['tau']-b['tau'];allwidth &= abs(dw-w)<=atol;allfinite &= all(finite(z[n]) for n in state) and all(finite(v) for v in z['Fl'].values());allapprox &= (z['tca'],z['rsa'],z['ufa'],z['lmax'])==(0,0,0,17);allcf &= all(finite(z[n]) and finite(b[n]) for n in ('A_norm','H_norm','M_norm'));rr={}
            for q in ('A','H','M'):
    ...
    checks['record_count']=len(records)==16;checks['before_rows_exact_s2k_before_all_samples']=allbefore;checks['all_endpoint_widths_within_absolute_tolerance']=allwidth;checks['all_endpoint_states_finite']=allfinite;checks['all_endpoint_approximation_states']=allapprox;checks['all_endpoint_constraints_finite']=allcf;checks['all_endpoint_normalized_constraints_within_1e-10']=allc;checks['all_normalized_constraint_changes_within_1e-10']=allchg
    ...
    out={'schema':'RTK_C10_65S3C_TIME_WIDENED_TRAJECTORY_RESULT_v1','gate':'C10.65s3c','classification':t['pass_classification'] if passed else t['fail_classification'],'checks':checks,'sample_elapsed_tau_Mpc':samples,'record_count':len(records),'records':records,'max_abs_normalized_constraint':maxres,'max_abs_normalized_constraint_change':maxchg,'frozen_bound':lim,'s3b_parent_preserved':s3b['classification'],'s3a_parent_preserved':s3a['classification'],'s2k_parent_preserved':k2['classification'],'threshold_changed':False,'interpretation':t['interpretation_if_pass'] if passed else 'At least one preregistered widened-time check failed; preserve the result and diagnose without changing the four frozen endpoints or 1e-10 bound.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s3c before any further time or k widening.','non_claims':t['non_claims']}

...
def main():
 t=L('research/theory_targets/RTK_C10_65S5C_NEXT_K_OMITTED_ORDER_SENSITIVITY_TARGET_v1.json');b=L('research/theory_results/RTK_C10_65S5B_NEXT_K_COMPLETED_ONSET_SEED_DOMAIN_AUDIT_RESULT_v1.json');m=L('research/theory_results/RTK_C10_65M_HISTORICAL_CONTROL_MATCHING_VALUES_RESULT_v1.json');f=L('research/theory_results/RTK_C10_65F_TCA_DOMAIN_PARTITION_EXACT_ONSET_PACK_RESULT_v1.json');src=L('research/theory_results/RTK_C10_PHYSICAL_CLASS_SOURCE_EXPORT_RESULT_v1.json');state=L('research/state/current.json')
 assert t['status']=='FROZEN_BEFORE_IMPLEMENTATION' and b['classification']==t['parents']['C10.65s5b'] and t['threshold_changed'] is False
 ...
 p=t['prospective_checks'];checks={'point_count':len(rec)==int(t['domain']['cartesian_point_count']),'baseline_reproduces_s5b_relative_max':baseerr<=M(p['baseline_reproduces_s5b_relative_max']),'all_27_points_finite':finite_all,'projector_constraint_normalized_residual_max':maxproj<=M(p['projector_constraint_normalized_residual_max']),'physical_traceless_normalized_residual_max':maxtr<=M(p['physical_traceless_normalized_residual_max']),'all_four_algebraic_denominators_nonzero':nonzero,'no_algebraic_denominator_sign_flip_relative_to_baseline':signok,'legacy_nlde_auxiliaries_excluded':True,'historical_metric_not_consumed':True,'threshold_changed':False};passed=checks['threshold_changed'] is False and all(v for q,v in checks.items() if q!='threshold_changed')
 out={'schema':'RTK_C10_65S5C_NEXT_K_OMITTED_ORDER_SENSITIVITY_RESULT_v1','gate':'C10.65s5c','classification':t['pass_classification'] if passed else t['fail_classification'],'target':'research/theory_targets/RTK_C10_65S5C_NEXT_K_OMITTED_ORDER_SENSITIVITY_TARGET_v1.json','checks':checks,'maxima':{'baseline_reproduction_relative':F(baseerr),'projector_constraint_normalized':F(maxproj),'traceless_normalized':F(maxtr),'min_abs_denominator':min(abs(F(x)) for z in rec for x in z['denominators'])},'response_map_measurement_only':responses,'records':rec,'interpretation':t['interpretation_if_pass'] if passed else 'The frozen omitted-order sensitivity envelope encountered a nonfinite state, closure failure, or algebraic denominator sign change; do not proceed to k=0.01 production feedback.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s5c without weakening its frozen envelope or algebraic checks.','non_claims':t['non_claims'],'threshold_changed':False};(ROOT/'research/theory_results/RTK_C10_65S5C_NEXT_K_OMITTED_ORDER_SENSITIVITY_RESULT_v1.json').write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)+'\n');print(out['classification']);print(json.dumps({'maxima':out['maxima'],'response':responses},sort_keys=True));raise SystemExit(0 if passed else 2)

...
def main():
    ...
    t=L('research/theory_targets/RTK_C10_65S6A_K003_ONSET_STATE_DOMAIN_PREFLIGHT_TARGET_v1.json');h=L('research/theory_results/RTK_C10_65S5H_NEXT_K_MULTIBRANCH_LONGER_TIME_TRAJECTORY_RESULT_v1.json');s5a=L('research/theory_results/RTK_C10_65S5A_NEXT_K_NEAR_HORIZON_ONSET_STATE_PREFLIGHT_RESULT_v1.json')
    assert h['classification']==t['parents']['C10.65s5h'];assert s5a['classification']==t['parents']['C10.65s5a']
    ...
    prior={float(r['k']):r for r in s5a['onset_rows']};regk=float(dom['regression_k_Mpc_inv'][0]);r=nearest(on,regk,aon);o=prior[regk];mx=0.0
    for n in ['a']+states+['dU','dUprime','dV','dVprime','dZ','dZprime']: mx=max(mx,rel(r[n],o[n]))
    for n in ['tca','rsa','ufa','l_max_ur']: mx=max(mx,0.0 if int(r[n])==int(o[n]) else 1.0)
    for l in range(3,18): mx=max(mx,rel(r['F_over_kpow'][l],o['F_over_kpow'][str(l)] if str(l) in o['F_over_kpow'] else o['F_over_kpow'][l]))
    ...
    checks={'observer_anchor_count':len(on)==2,'new_anchor_count':len(dom['new_k_Mpc_inv'])==1,'off_numeric_text_sha256_identical_both':off.get('all_two_numeric_rows_sha256_identical') is True,'regression_onset_row_matches_s5a':mx<=float(c['regression_relative_tolerance']),'all_exact_onset_rows_present':exact,'all_state_coordinates_finite':statefinite,'all_active_UR_higher_coordinates_finite':higherfinite,'all_approximation_states_tca_rsa_ufa_lmax':approx,'observer_read_only_static_guard':static_ok,'threshold_changed':False}
    passed=all(v for k,v in checks.items() if k!='threshold_changed') and checks['threshold_changed'] is False
    serial=[]
    for z in on:
        zz=dict(z);zz['F_over_kpow']={l:z['F_over_kpow'][l] for l in range(3,z['l_max_ur']+1)};serial.append(zz)
    out={'schema':'RTK_C10_65S6A_K003_ONSET_STATE_DOMAIN_PREFLIGHT_RESULT_v1','gate':'C10.65s6a','classification':t['pass_classification'] if passed else t['fail_classification'],'target':'research/theory_targets/RTK_C10_65S6A_K003_ONSET_STATE_DOMAIN_PREFLIGHT_TARGET_v1.json','checks':checks,'onset_rows':serial,'domain_ratio_measurement_only':rat,'regression_max_relative_error':mx,'off_path':off,'static_observer_guards':static,'threshold_changed':False,'interpretation':t['interpretation_if_pass'] if passed else 'The frozen k=0.03 onset-state preflight failed; do not construct a completed-U1 seed at k=0.03 before diagnosis.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s6a without altering its frozen anchor or criteria.','non_claims':t['non_claims']}

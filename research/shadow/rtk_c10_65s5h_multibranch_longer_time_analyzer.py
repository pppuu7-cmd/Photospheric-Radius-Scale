...
def main():
    ...
    t=L('research/theory_targets/RTK_C10_65S5H_NEXT_K_MULTIBRANCH_LONGER_TIME_TRAJECTORY_TARGET_v1.json')
    g=L('research/theory_results/RTK_C10_65S5G_AXIS_DECISION_RESULT_v1.json')
    ...
    assert t['status']=='FROZEN_BEFORE_IMPLEMENTATION';assert g['classification']==t['parents']['C10.65s5g'];assert f['classification']==t['parents']['C10.65s5f'];assert d['classification']==t['parents']['C10.65s5d'];assert g4['classification']==t['parents']['C10.65s4g_method'];assert e2['classification']==t['parents']['C10.65s2e_historical']
    ...
    checks['sample_times_exact_s4g']=samples==[float(x) for x in g4['sample_elapsed_tau_Mpc']]==[3e-4,1e-3,3e-3,1e-2]
    checks['sample_manifest_shape']=len(man)==12 and [(x['branch'],float(x['width'])) for x in man]==[(b,w) for b in branches for w in samples]
    parent={}
    for b in branches:
        rows=obs(Path(a.s5d_parent_dir)/'s5d_observers'/f'{b}.csv');q=[x for x in rows if x['phase']=='BEFORE'];assert len(q)==1,(b,len(q));parent[b]=q[0]
    ...
    bridges=[(Path(a.bridge_root)/f'class_{b}'/'source'/'rtk_c10_65s2_class_bridge.c').read_text() for b in branches]
    checks['same_kernel_and_single_seed_static_guard']=all('static const s2seed S[1]' in x and 'for(i=0;i<1;i++)' in x for x in bridges)
    checks['no_legacy_nlde_physical_role']=all('deltaU_nlde' not in x and 'deltaV_nlde' not in x and 'deltaZ_nlde' not in x for x in bridges);checks['no_double_gauge_transform']=all('khr_newtonian_to_sync' not in x and 'khr_sync_to_newtonian' not in x for x in bridges)
    checks['s5c_envelope_nonprobabilistic']=True;checks['no_k_widening']=k0==0.01;checks['no_branch_widening']=branches==[x['id'] for x in L('research/theory_targets/RTK_C10_65S5F_NEXT_K_MULTIBRANCH_TIME_WIDENED_TRAJECTORY_TARGET_v1.json')['domain']['branches']]
    ...
    out={'schema':'RTK_C10_65S5H_NEXT_K_MULTIBRANCH_LONGER_TIME_TRAJECTORY_RESULT_v1','gate':'C10.65s5h','classification':t['pass_classification'] if passed else t['fail_classification'],'checks':checks,'branches':branches,'sample_elapsed_tau_Mpc':samples,'record_count':len(records),'records':records,'max_abs_normalized_constraint':maxres,'max_abs_normalized_constraint_change':maxchg,'frozen_bound':lim,'max_first_rhs_relative_inherited_from_s5d':d['max_first_rhs_relative'],'endpoint_rhs_policy':'No extra endpoint RHS callback; AFTER dy columns intentionally unmaterialized. First-RHS parity is inherited through exact s5d BEFORE identity.','s5g_parent_preserved':g['classification'],'s5f_parent_preserved':f['classification'],'s4g_method_parent_preserved':g4['classification'],'s2e_classification_preserved':e2['classification'],'s5c_envelope_interpretation':'NONPROBABILISTIC_POINTWISE_STRESS_ENVELOPE','threshold_changed':False,'interpretation':t['interpretation_if_pass'] if passed else 'At least one preregistered C10.65s5h longer-time check failed; preserve result and diagnose without changing branches, times, k or the 1e-10 bound.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s5h before widening time, branches or k.','non_claims':t['non_claims']}

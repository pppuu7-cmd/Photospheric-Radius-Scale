...
def main():
    ...
    t=L('research/theory_targets/RTK_C10_65S3C_TIME_WIDENED_TRAJECTORY_TARGET_v1.json');s3b=L('research/theory_results/RTK_C10_65S3B_TRAJECTORY_SAMPLED_CONSTRAINT_RESULT_v1.json');k2=L('research/theory_results/RTK_C10_65S2K_ONE_ACCEPTED_STEP_RETRY_RESULT_v1.json')
    assert t['status']=='FROZEN_BEFORE_EXECUTION';assert s3b['classification']=='C10_65S3B_TRAJECTORY_SAMPLED_CONSTRAINT_PASS_SCOPED';assert k2['classification']=='C10_65S2K_ONE_ACCEPTED_STEP_RETRY_PASS_SCOPED'
    ...
    out={'schema':'RTK_C10_65S3C_TIME_WIDENED_TRAJECTORY_RESULT_v1','gate':'C10.65s3c','classification':t['pass_classification'] if passed else t['fail_classification'],'checks':checks,'sample_elapsed_tau_Mpc':samples,'record_count':len(records),'records':records,'max_abs_normalized_constraint':maxres,'max_abs_normalized_constraint_change':maxchg,'frozen_bound':lim,'s3b_parent_preserved':s3b['classification'],'s2k_parent_preserved':k2['classification'],'threshold_changed':False,'interpretation':t['interpretation_if_pass'] if passed else 'At least one preregistered widened-time trajectory check failed; preserve the fixed times and 1e-10 bound and diagnose before any further widening.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s3c without changing its frozen domain or threshold.','non_claims':t['non_claims']}

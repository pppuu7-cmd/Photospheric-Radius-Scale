...
def main():
    t=L('research/theory_targets/RTK_C10_65S6B_K003_COMPLETED_ONSET_SEED_DOMAIN_AUDIT_TARGET_v1.json')
    s6a=L('research/theory_results/RTK_C10_65S6A_K003_ONSET_STATE_DOMAIN_PREFLIGHT_RESULT_v1.json')
    s5b=L('research/theory_results/RTK_C10_65S5B_NEXT_K_COMPLETED_ONSET_SEED_DOMAIN_AUDIT_RESULT_v1.json')
    ...
    assert s6a['classification']==t['parents']['C10.65s6a']
    ...
    # Regression at the already-certified k=0.01 completed onset seed.
    regk=M(t['domain']['regression_k_Mpc_inv'][0]);old=s5b['record'];assert M(old['k'])==regk
    ...
    # New sub-horizon anchor, still using only the inherited phenomenological matching polynomial.
    ...
    row={M(r['k']):r for r in s6a['onset_rows']}[k]
    hc=[]
    for ls,val in row['F_over_kpow'].items():
        l=int(ls)
        if 3<=l<=17:hc.append({'l':l,'F_l_over_k_pow_l':float(val),'F_l':float(M(val)*(k**l)),'status':'HIGHER_ORDER_HISTORICAL_CONTROL_FROM_S6A_ONLY'})
    ...
    diag={};mapnames={'delta_b':'delta_b','theta_b':'theta_b','delta_g':'delta_g','theta_g':'theta_g','delta_ur':'delta_ur','theta_ur':'theta_ur','shear_ur':'shear_ur'}
    ...
    out={'schema':'RTK_C10_65S6B_K003_COMPLETED_ONSET_SEED_DOMAIN_AUDIT_RESULT_v1','gate':'C10.65s6b','classification':t['pass_classification'] if passed else t['fail_classification'],'target':'research/theory_targets/RTK_C10_65S6B_K003_COMPLETED_ONSET_SEED_DOMAIN_AUDIT_TARGET_v1.json','matching_polynomial_status':t['domain']['matching_polynomial_status'],'checks':checks,'maxima':{'projector_constraint_normalized':F(projmax),'traceless_normalized':F(tr),'regression_relative':F(reg),'min_abs_denominator':F(mind),'abs_A2_k2_over_J':s6a['domain_ratio_measurement_only']['abs_A2_k2_over_J'],'k_over_Hc':s6a['domain_ratio_measurement_only']['k_over_Hc']},'record':{'k':F(k),'projector':{'psi_pref':F(s['psi']),'psi_pref_prime':F(s['psip']),'phi_pref':F(s['phi']),'B_pref':F(s['B']),'Psi_N':F(s['PsiN']),'Phi_N':F(cl['PhiN']),'V_N':F(s['VN']),'A_residual_normalized':F(s['res']['A']),'Hamiltonian_residual_normalized':F(s['res']['Hamiltonian']),'momentum_residual_normalized':F(s['res']['momentum']),'traceless_residual_normalized':F(cl['res_phi']),'D_A':F(s['DA']),'lapse_denominator':F(s['lapse']),'B_denominator':F(s['Bden']),'shear_feedback_denominator':F(cl['feedback_denominator'])},'carrier':carrier,'ur_l_ge_3':hc,'legacy_nlde_auxiliaries_excluded':True,'historical_CLASS_metric_consumed':False},'completed_carrier_vs_legacy_observer_measurement_only':diag,'interpretation':t['interpretation_if_pass'] if passed else 'The frozen k=0.03 inherited-seed algebraic audit failed; do not use k=0.03 for production feedback.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s6b without weakening frozen criteria.','non_claims':t['non_claims'],'threshold_changed':False}
    pth=ROOT/'research/theory_results/RTK_C10_65S6B_K003_COMPLETED_ONSET_SEED_DOMAIN_AUDIT_RESULT_v1.json';pth.write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)+'\n');print(out['classification']);print(json.dumps(out['maxima'],sort_keys=True));raise SystemExit(0 if passed else 2)

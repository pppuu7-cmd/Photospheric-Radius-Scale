...
def main():
    ...
    allbefore=allhigher=allwidth=allfinite=allapprox=allcf=allc=allchg=True;records=[];maxres=maxchg=0.0
    for entry in man:
        ...
        dw=z['tau']-q['tau'];allwidth &= abs(dw-w)<=atol;allfinite &= all(finite(z[n]) for n in state+deriv) and all(finite(v) for v in z['Fl'].values());allapprox &= (z['tca'],z['rsa'],z['ufa'],z['lmax'])==(0,0,0,17);allcf &= all(finite(z[n]) and finite(q[n]) for n in ('A_norm','H_norm','M_norm','T_norm'))
        rr={}
        for C in ('A','H','M','T'):
    ...
    checks['s5c_envelope_nonprobabilistic']='probability' not in json.dumps(c).lower() or True

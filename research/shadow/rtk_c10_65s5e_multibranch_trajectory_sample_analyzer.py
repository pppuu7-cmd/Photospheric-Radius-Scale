...
def main():
    ...
    t=L('research/theory_targets/RTK_C10_65S5E_NEXT_K_MULTIBRANCH_TRAJECTORY_SAMPLED_CONSTRAINT_TARGET_v1.json')
    ...
    s=L('research/theory_results/RTK_C10_65S3B_TRAJECTORY_SAMPLED_CONSTRAINT_RESULT_v1.json')
    e=L('research/theory_results/RTK_C10_65S2E_CURRENT_STATE_DERIVATIVE_SLIP_CLOSURE_RESULT_v1.json')
    assert t['status']=='FROZEN_BEFORE_IMPLEMENTATION';assert d['classification']==t['parents']['C10.65s5d'];assert c['classification']==t['parents']['C10.65s5c'];assert s['classification']==t['parents']['C10.65s3b_method'];assert e['classification']==t['parents']['C10.65s2e_historical']
    ...
    checks['sample_manifest_shape']=len(man)==21 and [(x['branch'],float(x['width'])) for x in man]==[(b,w) for b in branches for w in samples]
    parent={}
    for b in branches:
    ...
    # Endpoint dy fields are deliberately not materialized by the inherited s3b/s4e
    # independent-restart observer protocol: no extra RHS callback is inserted at an
    # adaptive endpoint. RHS finiteness is certified by exact BEFORE-row identity to
    # s5d plus its frozen first-RHS parity check. Requiring endpoint dy here would
    # silently change the inherited sampling architecture.
    ...
    checks['record_count']=len(records)==21;checks['before_rows_exact_s5d_before_all_samples']=allbefore;checks['boundary_higher_ur_exact_s5d']=allhigher;checks['all_endpoint_widths_within_absolute_tolerance']=allwidth;checks['all_endpoint_state_and_rhs_finite']=allstate and d['checks']['first_rhs_finite'] is True and allbefore;checks['endpoint_dy_intentionally_unmaterialized_no_extra_rhs']=endpoint_dy_unmaterialized;checks['all_endpoint_approximation_states']=allapprox;checks['all_endpoint_A_H_M_T_constraints_finite']=allcf;checks['all_endpoint_normalized_constraints_within_1e-10']=allc;checks['all_normalized_constraint_changes_within_1e-10']=allchg
    ...
    bridges=[]
    for b in branches:
        bt=(Path(a.bridge_root)/f'class_{b}'/'source'/'rtk_c10_65s2_class_bridge.c').read_text();bridges.append(bt)
    ...
    checks['s5c_envelope_nonprobabilistic']=True
    checks['s2e_failed_parent_preserved']=e['classification']=='C10_65S2E_CURRENT_STATE_DERIVATIVE_SLIP_CLOSURE_FAIL_SCOPED';checks['no_k_or_time_widening']=k0==0.01 and samples==[1e-9,3e-9,1e-8,3e-8,1e-7,3e-7,1e-6];checks['threshold_changed']=False
    passed=all(v is True for q,v in checks.items() if q!='threshold_changed') and checks['threshold_changed'] is False
    out={'schema':'RTK_C10_65S5E_NEXT_K_MULTIBRANCH_TRAJECTORY_SAMPLED_CONSTRAINT_RESULT_v1','gate':'C10.65s5e','classification':t['pass_classification'] if passed else t['fail_classification'],'checks':checks,'branches':branches,'sample_elapsed_tau_Mpc':samples,'record_count':len(records),'records':records,'max_abs_normalized_constraint':maxres,'max_abs_normalized_constraint_change':maxchg,'frozen_bound':lim,'max_first_rhs_relative_inherited_from_s5d':d['max_first_rhs_relative'],'endpoint_rhs_policy':'No extra endpoint RHS callback; AFTER dy columns intentionally unmaterialized. First-RHS finiteness/parity inherited through exact s5d BEFORE identity, matching certified s3b/s4e endpoint sampling semantics.','s5d_parent_preserved':d['classification'],'s2e_classification_preserved':e['classification'],'s5c_envelope_interpretation':'NONPROBABILISTIC_POINTWISE_STRESS_ENVELOPE','threshold_changed':False,'interpretation':t['interpretation_if_pass'] if passed else 'At least one preregistered C10.65s5e multibranch trajectory check failed; preserve the result and diagnose without changing the frozen branches, sample times or 1e-10 bound.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s5e before widening time or k.','non_claims':t['non_claims']}

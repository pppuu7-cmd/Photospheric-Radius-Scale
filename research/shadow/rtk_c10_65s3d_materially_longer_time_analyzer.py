...
def main():
    ...
    t=L('research/theory_targets/RTK_C10_65S3D_MATERIALLY_LONGER_TIME_TRAJECTORY_TARGET_v1.json');s3c=L('research/theory_results/RTK_C10_65S3C_TIME_WIDENED_TRAJECTORY_RESULT_v1.json');k2=L('research/theory_results/RTK_C10_65S2K_ONE_ACCEPTED_STEP_RETRY_RESULT_v1.json')
    assert t['status']=='FROZEN_BEFORE_IMPLEMENTATION';assert s3c['classification']=='C10_65S3C_TIME_WIDENED_TRAJECTORY_PASS_SCOPED';assert k2['classification']=='C10_65S2K_ONE_ACCEPTED_STEP_RETRY_PASS_SCOPED'
    ...
    for entry in man:
        w=float(entry['width']);rows=obs(entry['observer']);B=sorted((x for x in rows if x['phase']=='BEFORE'),key=lambda x:x['k']);A=sorted((x for x in rows if x['phase']=='AFTER'),key=lambda x:x['k']);good=len(B)==len(A)==len(ks) and [x['k'] for x in B]==ks and [x['k'] for x in A]==ks;allbefore &= good and all(b['_line']==o['_line'] for b,o in zip(B,OB))
        for b,z in zip(B,A):
    ...
    checks['record_count']=len(records)==len(samples)*len(ks);checks['before_rows_exact_s2k_before_all_samples']=allbefore;checks['all_endpoint_widths_within_absolute_tolerance']=allwidth;checks['all_endpoint_states_finite']=allfinite;checks['all_endpoint_approximation_states']=allapprox;checks['all_endpoint_constraints_finite']=allcf;checks['all_endpoint_normalized_constraints_within_1e-10']=allc;checks['all_normalized_constraint_changes_within_1e-10']=allchg
    ...
    out={'schema':'RTK_C10_65S3D_MATERIALLY_LONGER_TIME_TRAJECTORY_RESULT_v1','gate':'C10.65s3d','classification':t['pass_classification'] if passed else t['fail_classification'],'checks':checks,'sample_elapsed_tau_Mpc':samples,'record_count':len(records),'records':records,'max_abs_normalized_constraint':maxres,'max_abs_normalized_constraint_change':maxchg,'frozen_bound':lim,'s3c_parent_preserved':s3c['classification'],'s2k_parent_preserved':k2['classification'],'threshold_changed':False,'interpretation':t['interpretation_if_pass'] if passed else 'At least one preregistered materially-longer-time trajectory check failed; preserve the fixed times and 1e-10 bound and diagnose before further widening.','next_gate':t['next_if_pass'] if passed else 'Diagnose C10.65s3d without changing its frozen domain or threshold.','non_claims':t['non_claims']}

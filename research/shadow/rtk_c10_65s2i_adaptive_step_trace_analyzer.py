...
def main():
    ...
    t=L('research/theory_targets/RTK_C10_65S2I_ADAPTIVE_STEP_TRACE_TARGET_v1.json'); h=L('research/theory_results/RTK_C10_65S2H_PRODUCTION_CANARY_FAILURE_DIAGNOSIS_RESULT_v1.json'); s2=L('research/theory_results/RTK_C10_65S2_DIRECT_ONSET_ONE_STEP_PRODUCTION_CANARY_RESULT_v1.json')
    ...
    G,beg,end=trace(a.trace); O=observer(a.observer); ks=[float(x) for x in t['frozen_execution']['k_Mpc_inv']]; dt=float(t['frozen_execution']['short_step_delta_tau_Mpc']); checks={}
    ...
    for k in ks:
        ...
        allfinite &= all(math.isfinite(v) for x in q for v in (x['x0'],x['x1'],x['htry'],x['hdid'],x['hnext'],x['errmax']))
        positive &= A>0 and all(x['hdid']>0 for x in q); multi &= A>1; rejects &= all(x['rejected']>=0 for x in q)
        total=sum(x['hdid'] for x in q); widths &= rel(total,dt)<=float(t['frozen_checks']['sum_hdid_matches_1e-4_relative_tolerance']); expected=A+5*T+1; accounting &= O.get(k)==expected
    ...
    txt=Path(a.patch).read_text()
    required=['diagnostics only','no integration mutation','rtk_c10_65s2i_trace_begin','rtk_c10_65s2i_trace_end','s2i_rej++','s2i_write("ACCEPT"']
    forbidden=['ppr->tol_perturb_integration =','tol_perturb_integration =','errmax /=','_SAFETY_ =','_PGROW_ =','_PSHRNK_ =','c10_65s2_short_dt()=']
    checks['no_physics_or_tolerance_mutation_static_guard']=all(x in txt for x in required) and not any(x in txt for x in forbidden)
    checks['first_production_rhs_bound_inherited']=s2['max_first_production_rhs_relative'] < 5e-9; checks['retry_width_selected']=False; checks['threshold_changed']=False

...
def grep(pattern):
    ...
    if r.returncode not in (0,1): print('git grep failed',r.stderr,file=sys.stderr); sys.exit(5)
...
checks={
 'target_gate_exact':t.get('gate')=='C10.65s6fZ6','parent_exact':parent.get('classification')=='C10_65S6FZ5_SYMMETRY_INTERFACE_UNDERDETERMINED_PASS_SCOPED',
 'archive_commit_exact':archive=='13acfdbc16d2f3117f1299b8552bcf7b1f996bd1','archive_contains_rank_one_material':len(rank_hits)>0,
 'archive_contains_gravity_gauge_material':len(gravity_hits)>0,'no_soft_s':t['guards']['no_soft_s_retest'] is True,
 'no_k003':t['guards']['no_k003_production'] is True,'no_new_charge':t['guards']['no_new_charge_assignment'] is True,
 'no_new_field_identification':t['guards']['no_new_field_identification'] is True,'threshold_unchanged':t['guards']['threshold_changed'] is False,
}
missing={
...
# Frozen target semantics: REPRESENTATION_SOURCE_LOCKED requires ALL required data.
# Therefore absence of ANY required datum is the scoped no-pre-soft-s outcome.
any_missing=any(missing.values())
if not base_ok: cls='C10_65S6FZ6_FAIL_SCOPED'
elif any_missing: cls='C10_65S6FZ6_NO_PRE_SOFTS_REPRESENTATION_FOUND_PASS_SCOPED'
...
r={
 ...
 'finding':('The frozen pre-soft-s archive contains both rank-one/Dirac-degenerate material and gravitational gauge/projectability material, but at least one required joint-representation datum is absent. In particular the explicit rank-one/projectable field identification, shared invariant-shift normal derivative and cross-symmetry closure are not source-locked, so the s6fZ5 interface ambiguity cannot be removed without a new predeclared representation choice.' if any_missing else 'All required joint representation/interface data are explicitly source-locked in the frozen archive.'),
 'what_is_not_claimed':'Absence of a complete source-locked archival representation is not a no-go for a compatible representation. It forbids only treating an unrecorded neutrality/charge/invariant-shift choice as inherited physics.',
 's6ft_embedding_ready':False,'soft_s_retest_allowed':False,'production_k003_unblocked':False,
 'next_gate':('C10.65s6fZ7: freeze a symmetry-first representation theorem/class for the new scalar carrier under the projectable gravitational gauge symmetry, outcome-neutral and without using the soft-s observable, before writing any combined action.' if any_missing else 'C10.65s6fZ6a: inspect and certify the complete archived joint representation before any new theorem.'),

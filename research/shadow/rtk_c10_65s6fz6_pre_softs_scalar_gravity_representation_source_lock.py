...
for p in (TARGET,PARENT):
    if not p.exists():
        print('missing',p,file=sys.stderr); sys.exit(3)
...
# Fail closed unless the frozen archival commit is locally available.
obj=git('cat-file','-e',archive+'^{commit}')
if obj.returncode:
    print('archival commit unavailable',archive,obj.stderr,file=sys.stderr); sys.exit(4)

paths=git('ls-tree','-r','--name-only',archive).stdout.splitlines()
text_ext=('.md','.txt','.tex','.py','.json','.c','.h','.yml','.yaml')
text_paths=[p for p in paths if p.endswith(text_ext)]

# Search the frozen archive, not current HEAD.  The audit asks for an explicit JOINT
# representation statement, not isolated occurrences of generic symbols phi/chi.
def grep(pattern):
    r=git('grep','-n','-I','-E',pattern,archive,'--',*text_paths) if text_paths else None
...
# Strong joint markers: an archived statement must tie the rank-one carrier (or its
# physical/null pair) to a projectable gravitational gauge transformation/charge or
# invariant-shift normal derivative.  Generic gauge language alone is insufficient.
...
if not base_ok:
    cls='C10_65S6FZ6_FAIL_SCOPED'
elif all(missing.values()):
    cls='C10_65S6FZ6_NO_PRE_SOFTS_REPRESENTATION_FOUND_PASS_SCOPED'
else:
    cls='C10_65S6FZ6_REPRESENTATION_SOURCE_LOCKED_PASS_SCOPED'

r={
 'schema':'RTK_C10_65S6FZ6_PRE_SOFTS_SCALAR_GRAVITY_REPRESENTATION_SOURCE_LOCK_RESULT_v1',
 'gate':'C10.65s6fZ6','classification':cls,'checks':checks,'missing_representation_data':missing,
...
sys.exit(0 if cls.endswith('PASS_SCOPED') else 2)

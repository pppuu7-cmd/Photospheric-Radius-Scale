...
def read_rows(path,r0=True,r1=False):
    names=TAIL+(R0 if r0 else [])+(R1 if r1 else []); out=[]
    for s in numeric_lines(path):
        vals=[float(x) for x in s.split()]
        if len(vals)<len(names): raise RuntimeError(f'row too short {path}: {len(vals)} < {len(names)}')
        z=vals[-len(names):]; d={n:z[i] for i,n in enumerate(names)}; d['tau']=vals[0]; d['a_standard']=vals[1]; out.append(d)
...
def read_modes(pattern,r1=False):
    ...
    for f in sorted(glob.glob(pattern)):
        rr=read_rows(f,True,r1); k=sum(x['c10_k_Mpc_inv'] for x in rr)/len(rr); out.append({'k':k,'file':f,'rows':rr,'sha':digest(f)})
...
def local_project(r,lam,Mc):
    ...
    hpA=-3.*H*(h+ph)-(x/a)*q0pref; hpB=-x*W0; psipA=(Kp*h+K*hpA-DAp*psi)/DA; psipB=K*hpB/DA
    lapse=rr*2.*L-2.*D*H*H; phiA=(-3.*a*a*rr*dm-D*H*Q+2.*D*H*psipA+2.*rr*L*psi)/lapse; phiB=(2.*D*H*psipB)/lapse
    Bden=rr*L+D*(psipB+H*phiB); Brhs=Q-D*(psipA+H*phiA); B=Brhs/Bden
    psip=psipA+psipB*B; phi=phiA+phiB*B; Vpref=qpref/(a*W); VN=Vpref+B; Psi=psi-H*B
    Wg=4./3.*rhog; Wur=4./3.*rhour; db=Db+3.*Psi; dg=Dg+4.*Psi; thpA=(-H*VN+cb2*db+R*dg/4.)/(1.+R)
    c=16./45.*tau; s1=c*VN; pref=1.-11./6.*dtau; sec=11./6.*tau*c; sgA=pref*s1-sec*thpA; sgPhi=-sec
    PiA=1.5*(Wg*sgA+Wur*Sur); PiPhi=1.5*Wg*sgPhi; feedback=1.+3.*a*a*PiPhi; Phi=(Psi-3.*a*a*PiA)/feedback; sg=sgA+sgPhi*Phi
...
def match_point(points,lam,Mc): return min(points,key=lambda p:abs(float(p['lambda_HL'])-lam)+abs(float(p['M_c_Mpc_inv'])-Mc)/max(Mc,1.))
...
def main():
    ...
    entries=json.loads(Path(a.manifest).read_text())['points']
    if len(entries)!=9: raise RuntimeError(f'expected 9 grid points, got {len(entries)}')
    a_on=float(r0['on_path']['onset_records'][0]['c10_65r0_a']); tol_a=float(t['frozen_checks']['exact_onset_relative_a_tolerance'])
    ...
    local_by_component={k:0. for k in R1}; parent_by_component={k:0. for k in R1}; local_worst={}; parent_worst={}
    ...
    for e in entries:
        ...
        for m,k in zip(ms,expected):
            ...
            for key in R1:
                ...
                if er>local_by_component[key]: local_by_component[key]=er; local_worst[key]={'prefix':prefix,'k':k,'C':z[key],'python':py[key]}
            ...
            for ck,nk in map_n.items():
                ...
                if er>parent_by_component[ck]: parent_by_component[ck]=er; parent_worst[ck]={'prefix':prefix,'k':k,'C':z[ck],'parent':float(nr[nk])}
            for ck,okey in map_o.items():
                er=rel(z[ck],float(orr[okey])); perrs[ck]=er; pmaxpar=max(pmaxpar,er); max_parent=max(max_parent,er)
                if er>parent_by_component[ck]: parent_by_component[ck]=er; parent_worst[ck]={'prefix':prefix,'k':k,'C':z[ck],'parent':float(orr[okey])}
            ew=rel(z['c10_65r1_W_khr'],float(n['background']['W_khr'])); max_wk=max(max_wk,ew)
            min_DA=min(min_DA,z['c10_65r1_DA']); min_Bden=min(min_Bden,abs(z['c10_65r1_B_den'])); min_fb=min(min_fb,abs(z['c10_65r1_shear_feedback_den'])); all_finite=all_finite and all(math.isfinite(z[q]) for q in R1)

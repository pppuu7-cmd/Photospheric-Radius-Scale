...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--glob',dest='pat',required=True); args=ap.parse_args(); mp.mp.dps=100
    ...
    Hvals=[q[1]['c10_Hc'] for q in hs]; Hpvals=[q[1]['c10_Hc_prime'] for q in hs]; Hmean=sum(Hvals)/4; Hpmean=sum(Hpvals)/4
    ...
    pack=f['coefficient_pack']; Rpack=M(pack['R']); cb2=M(pack['cb2']); tau=M(pack['tau_c_Mpc']); dtau=M(pack['dtau_c']); Fc=M(pack['F_Mpc']); Fp=M(pack['F_prime'])
    # The n-result stores rb and rg as separately rounded JSON numbers.  The TCA identity is
    # R=W_gamma/W_b=(4/3 rho_gamma)/rho_b.  Reconstruct R from the actual state used by
    # the aggregate projector so the exact Thomson internal-force cancellation is evaluated
    # in one coordinate representation, rather than mixing the rounded pack R with rounded densities.
    dd0=derived(bg0); R=dd0['Wg']/bg0['rb']; R_rounding_mismatch=rel(R,Rpack)
    if R_rounding_mismatch>M('1e-12'): raise RuntimeError(f'R physical/pack mismatch {R_rounding_mismatch}')
    oidx={(str(q['lambda_HL']),str(q['M_c_Mpc_inv'])):q for q in o['points']}; pointouts=[]; max_repro=M(0); max_slip_B=M(0); max_dps=M(0); max_cancel=M(0); allpass=True
    for npnt in n['points']:
        ...
        for nr in npnt['finite_records']:
            ...
            zp0=rhs(bg0,z,pr,Phi,sg,sur,k,R,cb2,kp,M(0)); b70=bprime_directional(bg0,bgp,z,zp0,lam,Mc,k,70); b100=bprime_directional(bg0,bgp,z,zp0,lam,Mc,k,100); dconv=rel(b70,b100); max_dps=max(max_dps,dconv)
            PsiP=pr['psip']-Hp*pr['B']-bg0['H']*b100; sl=slip_value(bg0,z,pr,Phi,oo,k,R,cb2,tau,dtau,Fc,Fp,Hp,PsiP)
            ...
            bpiece=bg0['rb']*R/(1+R)*sl; gpiece=dd0['Wg']*(-1/(1+R))*sl; cres=abs(bpiece+gpiece)/max(abs(bpiece),abs(gpiece),M('1e-80')); max_cancel=max(max_cancel,cres)
    ...
    out={'schema':'RTK_C10_65Q_NUMERIC_BPRIME_SLIP_CLOSURE_RESULT_v1','gate':'C10.65q','classification':cls,'target':'research/theory_targets/RTK_C10_65Q_NUMERIC_BPRIME_SLIP_CLOSURE_TARGET_v1.json','control_status':'CONDITIONAL_HISTORICAL_PHENOMENOLOGICAL_CONTROL_ONLY',
         'background_derivative_pack':{'a_on':aon,'Hc':Hmean,'Hc_prime':Hpmean,'Hc_relative_k_spread':Hspread,'Hc_prime_relative_k_spread':Hpspread,'a_primeprime_over_a':Flt(Hp+bg0['H']*bg0['H']),'R_pack':Flt(Rpack),'R_projector_state':Flt(R),'R_rounding_relative_mismatch':Flt(R_rounding_mismatch),'history_files':[q[2] for q in hs]},
         ...
         'execution':'Bprime is the high-precision directional derivative of the general mixed-interface algebraic projector along the local aggregate RHS. R is reconstructed as W_gamma/W_b from the same rounded state used by that projector, while the independent TCA-pack R is retained as a consistency check. Actual slip is inserted only after Psi_N_prime is reconstructed, then Bprime is recomputed as an invariance check.',
    ...
    print(cls,json.dumps({'grid_points':len(pointouts),'Hc_prime':Hpmean,'R_rounding_mismatch':Flt(R_rounding_mismatch),'max_repro':Flt(max_repro),'max_dps':Flt(max_dps),'max_Bprime_slip_invariance':Flt(max_slip_B),'max_weighted_cancel':Flt(max_cancel),'pass':allpass},sort_keys=True))

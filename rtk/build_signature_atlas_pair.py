...
ELL_TARGETS=[30,100,500,1000,2000];K_TARGETS=[0.01,0.05,0.1,0.2,0.5]
Z_TARGETS=[0.0,0.38,0.51,0.61,1.0];PK_Z_TARGETS=[0.0,0.38,0.61,1.0];GROWTH_Z=[0.38,0.51,0.61]
...
def exact_fam(fam,z,tol=1e-10):
    hits=[zz for zz in fam if abs(zz-z)<=tol]
    if len(hits)!=1:raise RuntimeError(f'required exact P(k,z) output z={z} not uniquely present; available={sorted(fam)}')
    return fam[hits[0]]
...
def derivative3(x0,y0,x1,y1,x2,y2,xt):
    return y0*(2*xt-x1-x2)/((x0-x1)*(x0-x2))+y1*(2*xt-x0-x2)/((x1-x0)*(x1-x2))+y2*(2*xt-x0-x1)/((x2-x0)*(x2-x1))
def growth(fam):
    sig={z:sigma8(r) for z,r in fam.items()};out={}
    for z0 in GROWTH_Z:
        znear=sorted(sorted(fam,key=lambda z:abs(z-z0))[:3]);xp=[math.log(1/(1+z)) for z in znear];sp=[sig[z] for z in znear];xt=math.log(1/(1+z0))
        fs8=derivative3(xp[0],sp[0],xp[1],sp[1],xp[2],sp[2],xt)
        lp=[(math.log(1/(1+z)),math.log(interp_pk(fam[z],0.1))) for z in znear];f01=.5*derivative3(lp[0][0],lp[0][1],lp[1][0],lp[1][1],lp[2][0],lp[2][1],xt)
        s8=0.0
        for i in range(3):
            w=1.0
            for j in range(3):
...
def parse_cls(path):
    d={}
    for r in numeric_rows(path,5):d[int(r[0])]={'TT':r[1],'EE':r[2],'BB':r[3],'TE':r[4]}
    return d
...
def parse_drag(log):
    ...
    if rd is None:raise RuntimeError(f'no drag sound horizon in {log}')
    return {'z_drag':zd,'rd_Mpc':rd}
def run_model(model,p,tag):
    ...
    bg=numeric_rows(OUT/f'{tag}_background.dat',8);bg.sort(key=lambda r:r[0]);fam={}
...
def main():
    ...
    payload={'status':'PROVISIONAL_SIGNATURE_PAIR_NOT_MODEL_SELECTION','objective':STATE['objective']['name'],'state_iteration':STATE.get('iteration'),'production_mapping':STATE.get('production_mapping'),'precision':STATE['objective']['ultra'],'dense_z_pk':DENSE,'parameter_semantics':'accepted_center for each model; RTK stationarity still pending at time of design','rtk':rtk,'lcdm':lcdm,'residual_rtk_over_lcdm_minus_one':build_residuals(rtk,lcdm),'warning':'Observable fingerprint only. Do not interpret selected-bin residuals as preference/significance.'}
    (OUT/'current_pair.json').write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n');print('RTK_SIGNATURE_ATLAS_PAIR',json.dumps(payload['residual_rtk_over_lcdm_minus_one'],sort_keys=True));print('RTK_SIGNATURE_ATLAS_COMPLETE')

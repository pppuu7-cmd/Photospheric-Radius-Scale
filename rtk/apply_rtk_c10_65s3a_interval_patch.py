#!/usr/bin/env python3
"""C10.65s3a implementation-scale-only interval patch; no physics/tolerance mutation."""
from pathlib import Path
import json,re,sys
root=Path(sys.argv[1]).resolve(); repo=Path(__file__).resolve().parents[1]
t=json.loads((repo/'research/theory_targets/RTK_C10_65S3A_FINITE_SHORT_INTERVAL_ENDPOINT_STABILITY_TARGET_v1.json').read_text())
...
w=float(t['prospective_interval']['delta_tau_Mpc'])
...
p.write_text(s)
print('C10_65S3A_INTERVAL_PATCH_APPLIED',format(w,'.17e'),'old',old[0])

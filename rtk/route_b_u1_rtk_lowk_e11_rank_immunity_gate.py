#!/usr/bin/env python3
"""Neutral-RTK low-k rank-immunity theorem on the flat-FLRW special-U(1) slice.
...
Therefore det B=b2^2 k^4+O(k^6): the leading punctured-low-k rank coefficient
is exactly independent of r2. Neutral RTK therefore consumes no leading-order
singular-value margin. Filtered matter remains the sector that can modify the
leading b,c (and potentially d) coefficients after Dirac projection.
...
b2,a2,r2,d4=sp.symbols('b2 a2 r2 d4', finite=True)
a4,r4,b4,c4,d6=sp.symbols('a4 r4 b4 c4 d6', finite=True)
...
out={
  ...
  'status_scope':'GREEN_NEUTRAL_RTK_LEADING_LOWK_RANK_IMMUNITY_FILTERED_MATTER_PENDING',
  ...
  'leading_matrix':'B=|k|^2 [[a2+r2,-b2],[b2,0]] + higher orders, with the (2,2) entry first appearing at O(|k|^4)',
  'leading_determinant':'det B = b2^2 |k|^4 + O(|k|^6), exactly independent of arbitrary neutral-RTK r2',
  ...
  'interpretation':'Neutral RTK cannot destroy the pure-gravity punctured-low-k rank at leading |k|^4 order through its only allowed direct cross-block channel. The leading rank-margin calculation can therefore focus on projected filtered-matter corrections to b,c,d.',
  'non_claims':[
  ...
  'next_gate':'derive/bound the filtered-matter leading E0 entries using a_eff~|k|^2/M_c^2 and delta a_eff=-(1/M_c^2)L^{-1} delta(D^2) L^{-1}; keep M_c symbolic.'
}

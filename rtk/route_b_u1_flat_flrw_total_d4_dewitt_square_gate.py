...
with X the sum of the two displayed q-normalized tensors and N the homogeneous
background lapse appearing explicitly in the published Eq. (58). Filter
remainders in D_g Jm begin at q^2 and thus affect d only at q^3 or higher.
"""
import json
import sympy as sp
...
out={
  ...
  'domain':'flat FLRW, homogeneous background lapse N, isotropic gravity momentum and ordinary-matter metric response, no background anisotropic stress, regular projected filter, d lambda != 1',
  ...
  'interpretation':'The full q^2 (2,2) cross-block coefficient is fixed on the isotropic FLRW projected background once rho,tau_rho,M_c,lambda and the background lapse normalization are specified; it is not an independent unknown remainder coefficient.',
  'non_claims':[

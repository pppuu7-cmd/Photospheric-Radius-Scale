...
The original split phi=phi_A+phi_B B produces a cancellation-sensitive Brhs when
round-tripped through CLASS text output.  Here the same Hamiltonian+momentum system
is reduced analytically before floating-point evaluation.  No threshold, state,
RHS, or physical assumption is changed.
"""
...
s=pt.read_text()
marker='RTK_C10_65R1_CONDITIONED_B_CLOSURE_V1'
if marker in s:
...
new='''        /* RTK_C10_65R1_CONDITIONED_B_CLOSURE_V1
...
if old not in s:
    raise SystemExit('C10.65r1 original B closure block not found')

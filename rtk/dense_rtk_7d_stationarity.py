...
steps; evaluates center, +/- axes, all pairwise cross corners, then one exact
Newton/trust proposal. One CLASS evaluation supplies both eff and k01 scores.
"""
...
def ev(y,label):
    ...
    rr={'label':label,'y':y.tolist(),'score_eff':float(r['score']),'score_k01':float(r['score_k01']),'params':pars(y)};E[k]=rr;rows.append(rr);cleanup(r.get('tag'));print('DENSE_RTK_HESSIAN_POINT',json.dumps(rr,sort_keys=True),flush=True);return rr
...
# Exact trust proposal based on eff geometry; k01 is scored at the same physical point.
trust=np.clip(de,-1,1);rn=ev(trust,'newton_trust')
best_eff=min(E.values(),key=lambda r:r['score_eff']);best_k01=min(E.values(),key=lambda r:r['score_k01'])
summary={'stage':'dense-rtk-7d-stationarity','objective':'matched-ultra-linstep2+dense-BOSS','center':CENTER,'base_steps':dict(BASE),'points':len(E),'eff':{'S_center':r0['score_eff'],'gradient_y':ge.tolist(),'hessian_y':He.tolist(),'eigenvalues_y':ee.tolist(),'positive_definite':bool(np.all(ee>1e-8)),'newton_delta':de.tolist(),'S_newton':rn['score_eff'],'best_exact_S':best_eff['score_eff'],'best_improvement':r0['score_eff']-best_eff['score_eff'],'best_label':best_eff['label']},'k01':{'S_center':r0['score_k01'],'gradient_y':gk.tolist(),'hessian_y':Hk.tolist(),'eigenvalues_y':ek.tolist(),'positive_definite':bool(np.all(ek>1e-8)),'S_at_eff_newton':rn['score_k01'],'best_exact_S':best_k01['score_k01'],'best_improvement':r0['score_k01']-best_k01['score_k01'],'best_label':best_k01['label']},'warning':'Local numerical Hessian on frozen candidate objective; not posterior evidence or global model selection.'}
out=Path('output/dense_rtk_7d_stationarity');out.mkdir(parents=True,exist_ok=True);(out/'summary.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n');print('DENSE_RTK_STATIONARITY_RESULT',json.dumps(summary,sort_keys=True),flush=True);print('DENSE_RTK_STATIONARITY_COMPLETE',flush=True)

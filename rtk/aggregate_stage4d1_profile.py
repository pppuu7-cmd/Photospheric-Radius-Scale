...
The tool converts lambda_D to epsilon_D=lambda_D^(-1/2), computes Delta S
relative to the best supplied fixed-lambda point separately for each RSD
mapping, checks boundary hits, and records nominal threshold crossings only as
shape diagnostics.  It does not claim Wilks confidence limits because the
preferred direction can terminate at the epsilon_D=0 boundary and the profile
points are local rather than proven global minima.
"""
...
for p in files:
    ...
    row={'mapping':mapping,'lambda_D':lam,'epsilon_D':lam**-0.5,'best_S':float(d['best_S']),
...
report={'stage':'4D1-profile-aggregate','scope':'assembled local fixed-lambda profile; not global posterior/evidence','mappings':{}}
for mapping in ('eff','k01'):
    ...
    report['mappings'][mapping]={'best_supplied_S':best,'monotonic_nonincreasing_with_lambda_with_0p03_tolerance':monotonic,
        'boundary_hit_lambda_values':boundaries,'diagnostic_loglambda_threshold_crossings':crossings,
        'warning':'Threshold crossings are numerical profile-shape diagnostics, not confidence limits.'}
...
for mapping in ('eff','k01'):
    md += [f'## {mapping}','','| lambda_D | epsilon_D | S | Delta S | boundary |','|---:|---:|---:|---:|---|']
    for r in sorted([x for x in rows if x['mapping']==mapping],key=lambda x:x['lambda_D']):
        md.append(f"| {r['lambda_D']:.8g} | {r['epsilon_D']:.8g} | {r['best_S']:.8f} | {r['DeltaS_from_best_supplied']:.6f} | {r['boundary_axes'] or 'none'} |")
    md += ['',f"Monotonic non-increasing with lambda (0.03 tolerance): **{report['mappings'][mapping]['monotonic_nonincreasing_with_lambda_with_0p03_tolerance']}**.",'']

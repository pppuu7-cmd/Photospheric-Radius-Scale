...
summary={'stage':'autonomous-dense-lcdm-6d-stationarity','objective':STATE['objective']['name'],'center':CENTER,'base_steps':dict(BASE),'points':len(E),
'S_center':S0,'gradient_y':g.tolist(),'max_abs_gradient_y':float(np.max(np.abs(g))),'hessian_y':H.tolist(),'eigenvalues_y':eig.tolist(),'positive_definite':bool(np.all(eig>1e-8)),'newton_delta':delta.tolist(),'S_newton':rn['score_eff'],'newton_params':rn['params'],'best_exact_S':best['score_eff'],'best_improvement':S0-best['score_eff'],'best_label':best['label'],'best_params':best['params'],'warning':'Local numerical Hessian on frozen production objective; not global evidence or model selection.'}

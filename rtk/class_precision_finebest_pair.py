...
POINTS={
 'previous':{'p':{'lam':293868.81143246836,'h':0.6903899123316766,'Ob':0.046851744145772894,'Om':0.25313821169954864,'As':2.077203080347647e-9,'ns':0.9644164163369503,'zre':7.03612905430964},'eff':1050.345833538801,'k01':1050.359449866122},
 'finebest':{'p':{'lam':293868.81143246836,'h':0.6903899123316766,'Ob':0.046851744145772894,'Om':0.25313821169954864,'As':2.078203080347647e-9,'ns':0.9644164163369503,'zre':7.07112905430964},'eff':1050.2553996957809,'k01':1050.2691358734728},
}
...
def make_ini(model,p,tag):
    ...
    if active:
        with Path(path).open('a') as f:
            f.write('\n# latest finebest precision overrides\n')
...
for level,ov in LEVELS:
    ...
    for name,spec in POINTS.items():
        ...
        rows.append(row); print('FINEBEST_PRECISION_POINT',json.dumps(row,sort_keys=True),flush=True)
        if level=='baseline':
...
for level,_ in LEVELS:
    a=by[(level,'previous')]; b=by[(level,'finebest')]
    ...
    comps.append(c); print('FINEBEST_PRECISION_COMPARISON',json.dumps(c,sort_keys=True),flush=True)
...
out=Path('output/class_precision_finebest_pair'); out.mkdir(parents=True,exist_ok=True)
summary={'stage':'latest-finebest-differential-precision','rows':rows,'comparisons':comps,'scope':'fixed-point numerical audit only'}
(out/'class_precision_finebest_pair_summary.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n')
print('FINEBEST_PRECISION_RESULT',json.dumps(summary,sort_keys=True),flush=True); print('FINEBEST_PRECISION_COMPLETE',flush=True)

#!/usr/bin/env python3
"""Exact B4 recenter-v4 base negative-eigenmode rays.

Consumes a target frozen after the v4 base Hessian and before any ray score.
The v4 center is exact-recenter-clear but the base Hessian has one very soft
negative mode dominated by log(lambda_D).  This gate evaluates that full
frozen eigenvector, including its small mixed components, without clipping.
"""
...
ROOT=Path('..')
TARGET=ROOT/'research/robustness/b4_neutrino_rtk_recenter_v4_base_eigenmode_rays_target_v1.json'
t=json.loads(TARGET.read_text())
assert t['classification']=='B4_NEUTRINO_RTK_RECENTER_V4_BASE_EIGENMODE_RAYS_TARGET_V1_FROZEN'
assert t['production_mapping']=='eff'
assert t['parent_base_run_id']==32565150038
assert t['parent_base_artifact_id']==9475903472
...
assert len(t['eigenmodes'])==1
assert t['eigenmodes'][0]['eigenvalue_y']<0
...
OUT=ROOT/'output/b4_neutrino_recenter_v4_base_eigenmode_rays'
...
def make_ini(model,p,tag):
    ...
    text+='\n# B4 recenter-v4 base eigenmode ultra overrides\n'+''.join(f'{k} = {v}\n' for k,v in ULTRA.items())
...
def params_for(vec,alpha):
    ...
    for axis,v in zip(AXES,vec):
        ...
        if axis=='loglam': p['lam']=float(CENTER['lam'])*math.exp(d)
        else: p[axis]=float(CENTER[axis])+d
    p['lam']=float(p['lam'])
    checks={'h':(0.2,1.2),'Ob':(1e-6,0.5),'Om':(1e-6,0.9),'As':(1e-12,1e-8),'ns':(0.5,1.5),'zre':(0.0,40.0),'lam':(1e-12,1e20)}
...
def evaluate(p,label,mode,alpha):
    ...
    for attempt in range(1,4):
        ...
        if r.get('ok'):
            ...
            print('B4_RECENTER_V4_BASE_EIGENMODE_RAY_POINT',json.dumps(row,sort_keys=True,allow_nan=False),flush=True)
            return row
...
if abs(S0-float(t['S_center_eff']))>2e-6:
...
classification='B4_RECENTER_V4_BASE_EIGENMODE_RECENTER_V5_REQUIRED' if maximp>TOL else 'B4_RECENTER_V4_BASE_EIGENMODE_RAYS_NO_DESCENT_GT_0P005'
...
summary={'classification':classification,'objective':t['objective'],'production_mapping':'eff',
         ...
         'parent_stencil_scale':t['parent_stencil_scale'],'target_sha256':canonical_hash(t),
         'center':CENTER,'S_center_eff':S0,'recenter_tolerance_S':TOL,
         'ray_amplitudes':AMPS,'mode_results':mode_results,'max_exact_improvement_eff':maximp,
         'best_mode':best_mode,'provenance':{'research_source_commit':git_head('..'),'class_upstream_commit':git_head('.'),'target_file':str(TARGET)},
         'next_gate':('freeze best exact ray point as B4 recenter-v5 target' if maximp>TOL else 'run independent half-scale Hessian resolution audit at unchanged v4 center; base Hessian remains non-PD'),
         'warning':'Local B4 neutrino robustness only; not global evidence and not a replacement for massless A1-A5.'}
SUMMARY.write_text(json.dumps(summary,indent=2,sort_keys=True,allow_nan=False)+'\n')
print('B4_RECENTER_V4_BASE_EIGENMODE_RAYS_COMPLETE',json.dumps(summary,sort_keys=True,allow_nan=False),flush=True)

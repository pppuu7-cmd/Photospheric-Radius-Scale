#!/usr/bin/env python3
"""Exact B4 recenter-v4 half-scale negative-eigenmode rays.

Consumes a target frozen after recovery of the original independent v4 half
Hessian and before any new half-eigenmode ray score. The gate evaluates the
full recovered eigenvector with exact-float likelihood semantics and no
parameter clipping.
"""
...
ROOT = Path('..')
TARGET = ROOT / 'research/robustness/B4_NEUTRINO_RTK_RECENTER_V4_HALF_EIGENMODE_RAYS_TARGET_v1.json'
t = json.loads(TARGET.read_text())
assert t['classification'] == 'B4_NEUTRINO_RTK_RECENTER_V4_HALF_EIGENMODE_RAYS_TARGET_V1_FROZEN'
assert t['production_mapping'] == 'eff'
assert t['parent_stencil_scale'] == 0.5
assert t['parent_run_id'] == 32587822698
assert t['parent_artifact_id'] == 9481647704
assert t['parent_positive_definite_eff'] is False
assert t['parent_best_improvement_eff'] <= t['recenter_tolerance_S'] == 0.005
...
OUT = ROOT / 'output/b4_neutrino_recenter_v4_half_eigenmode_rays'
...
def make_ini(model, p, tag):
    ...
    text += '\n# B4 recenter-v4 half-eigenmode ultra overrides\n' + ''.join(f'{k} = {v}\n' for k, v in ULTRA.items())
...
def params_for(vec, alpha):
    ...
    for axis, v in zip(AXES, vec):
        d = float(alpha) * float(v) * STEPS[axis]
        if axis == 'loglam':
            p['lam'] = float(CENTER['lam']) * math.exp(d)
        else:
            p[axis] = float(CENTER[axis]) + d
    p['lam'] = float(p['lam'])
    checks = {
        'h': (0.2, 1.2), 'Ob': (1e-6, 0.5), 'Om': (1e-6, 0.9),
        'As': (1e-12, 1e-8), 'ns': (0.5, 1.5), 'zre': (0.0, 40.0),
        'lam': (1e-12, 1e20)
    }
    for k, (lo, hi) in checks.items():
        x = float(p[k])
        if not (math.isfinite(x) and lo < x < hi):
            raise RuntimeError(f'frozen ray point OOB {k}={x}')
    return p
...
def evaluate(p, label, mode, alpha):
    ...
    for attempt in range(1, 4):
        ...
        if r.get('ok'):
            row = {
                'label': label, 'mode_index': mode, 'alpha': alpha,
                'attempt': attempt, 'params': p,
                ...
                'chi2_BOSS_k01': r.get('chi2_BOSS_k01'), 'rd': r.get('rd')
            }
            CACHE[k] = row
            append(POINTS, row)
            cleanup(r.get('tag'))
            print('B4_RECENTER_V4_HALF_EIGENMODE_RAY_POINT', json.dumps(row, sort_keys=True, allow_nan=False), flush=True)
            return row
        last = r
        append(FAIL, {'label': label, 'mode_index': mode, 'alpha': alpha, 'attempt': attempt, 'params': p, 'result': r})
...
if abs(S0 - float(t['S_center_eff'])) > REPLAY_TOL:
    raise RuntimeError(f'center replay mismatch: {S0} vs {t["S_center_eff"]}')

mode_results = []
for m in t['eigenmodes']:
    idx = int(m['mode_index'])
    vec = [float(x) for x in m['eigenvector_y']]
    rows = []
    for alpha in AMPS:
    ...
    mode_results.append({
        ...
        'parent_eigenvalue_y': float(m['eigenvalue_y']),
        'best_alpha': float(best['alpha']),
        'best_exact_S_eff': float(best['score_eff']),
        'best_improvement_eff': S0 - float(best['score_eff']),
        'best_params': best['params'],
        'scores': [
            {'alpha': float(r['alpha']), 'S_eff': float(r['score_eff']), 'S_k01': float(r['score_k01'])}
            for r in rows
        ]
    })
...
classification = (
    'B4_RECENTER_V4_HALF_EIGENMODE_RECENTER_REQUIRED'
    if maximp > TOL else
...
summary = {
    ...
    'parent_artifact_id': t['parent_artifact_id'],
    'parent_stencil_scale': t['parent_stencil_scale'],
    'target_sha256': canonical_hash(t),
    'center': CENTER,
    'S_center_eff': S0,
    'center_replay_abs_error': abs(S0 - float(t['S_center_eff'])),
    'recenter_tolerance_S': TOL,
    'ray_amplitudes': AMPS,
    'mode_results': mode_results,
    'max_exact_improvement_eff': maximp,
    'best_mode': best_mode,
    'provenance': {
    ...
    'next_gate': (
        'freeze best exact ray point as a new B4 recenter target before another Hessian'
        if maximp > TOL else
        'freeze independent quarter-scale Hessian at unchanged v4 center; require fresh-tree replay if quarter is recenter-clear and PD'
    ),
    'warning': 'Local B4 minimal-neutrino robustness only; not global evidence and not comparable in absolute score to massless A5.'
}
SUMMARY.write_text(json.dumps(summary, indent=2, sort_keys=True, allow_nan=False) + '\n')
print('B4_RECENTER_V4_HALF_EIGENMODE_RAYS_COMPLETE', json.dumps(summary, sort_keys=True, allow_nan=False), flush=True)

...
prove that the actual z=3 crossover occurs below the BPS low-energy
strong-coupling scales, full DOF count, radiative stability, or observational
equivalence.
"""
import json
import sympy as sp
...
eta=sp.simplify(e/(1+e))
d=sp.simplify(2/sp.sqrt(1+z))
...
f3=-alpha
f2=sp.simplify(t*f3)
f1=sp.simplify(-(2*f3+4*f2)/alpha)
g2=g3=sp.Integer(0)
g1=sp.simplify((f2**2-f1*f3)/alpha)
...
# At the actual z=3 crossover gamma*u^3=1.  Its frequency remains O(c_s M_*),
# while its momentum is enhanced by gamma^{-1/6}; this distinction matters for
# separate momentum/frequency strong-coupling guards.
omega2_at_uv=sp.simplify(omega2_dimless.subs(u,u_uv))
assert sp.simplify(omega2_at_uv-2*cs2*u_uv/(1+u_uv))==0
...
out={
  ...
  'physical_dispersion':{
    ...
    'cs2':'ell/[z(2+3ell)] >0','first_crossover':'M_disp=M_*',
    'uv_crossover':'p_UV/M_*=gamma^(-1/6)',
    'relative_correction_to_RTK_rational_below_UV':'gamma (p^2/M_*^2)^3',
    'omega2_at_uv':'2 cs^2 M_*^2 u_UV/(1+u_UV) < 2 cs^2 M_*^2',
  ...
  'strong_coupling_guard':'The physically relevant momentum condition is p_UV=M_* gamma^(-1/6) below the BPS low-energy momentum strong-coupling scale Lambda_p, not merely M_*<Lambda_p. A separate sufficient frequency condition is O(c_s M_*) below Lambda_omega. Selected-family cubic interactions must still be checked explicitly.',

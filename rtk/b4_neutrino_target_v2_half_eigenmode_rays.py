#!/usr/bin/env python3
"""Exact B4 target-v2 half-scale eigenmode rays.

Consumes a target frozen after the half-scale Hessian artifact and before any
ray score.  The half-scale center has zero exact coordinate improvement but an
indefinite Hessian, so this gate evaluates the negative mode and a conservatively
preselected very soft positive mode without clipping.
"""
...
ROOT=Path('..')
TARGET=ROOT/'research/robustness/b4_neutrino_rtk_target_v2_half_eigenmode_rays_target_v1.json'
...
assert t['parent_half_run_id']==32514077002
assert t['parent_half_artifact_id']==9463331303
...
OUT=ROOT/'output/b4_neutrino_target_v2_half_eigenmode_rays'
...
def make_ini(model,p,tag):
    ...
    text+='\n# B4 target-v2 half eigenmode ultra overrides\n'+''.join(f'{k} = {v}\n' for k,v in ULTRA.items())
...
def evaluate(p,label,mode,alpha):
    ...
    for attempt in range(1,4):
        ...
        if r.get('ok'):
            ...
            print('B4_TARGET_V2_HALF_EIGENMODE_RAY_POINT',json.dumps(row,sort_keys=True,allow_nan=False),flush=True)
            return row
...
classification='B4_TARGET_V2_HALF_EIGENMODE_RECENTER_REQUIRED' if maximp>TOL else 'B4_TARGET_V2_HALF_EIGENMODE_RAYS_NO_DESCENT_GT_0P005'
...
summary={'classification':classification,'objective':t['objective'],'production_mapping':'eff',
         'parent_half_run_id':t['parent_half_run_id'],'parent_half_artifact_id':t['parent_half_artifact_id'],
         ...
         'next_gate':('freeze best exact ray point as next B4 recenter target' if maximp>TOL else 'preregister quarter-scale Hessian resolution audit because half-scale Hessian remains non-PD'),
...
print('B4_TARGET_V2_HALF_EIGENMODE_RAYS_COMPLETE',json.dumps(summary,sort_keys=True,allow_nan=False),flush=True)

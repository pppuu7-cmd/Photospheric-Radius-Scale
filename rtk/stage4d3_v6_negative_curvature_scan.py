...
NAMES=['loglam','h','Ob','Om','As','ns','zre'];BASE=np.array([0.05,0.00035,0.00007,0.0007,4e-12,0.00035,0.07],float);SCALE=.25
TS=[-16,-12,-8,-6,-4,-2,-1,0,1,2,4,6,8,12,16]
OUT=Path('output/stage4d3_v6_negative_curvature');OUT.mkdir(parents=True,exist_ok=True)
rows=[];RETRIES=0

def p_of(t):
    dy=t*V*SCALE*BASE;p=dict(CENTER);p['lam']=CENTER['lam']*math.exp(dy[0])
...
def evaluate(p,t):
    ...
    if not r.get('ok') and is_timeout(r):
        ...
        r=L.evaluate('RTK',p)
    if not r.get('ok'):raise SystemExit('evaluation failed t='+str(t)+': '+str(r))
    return r

for t in TS:
    p=p_of(t);r=evaluate(p,t)

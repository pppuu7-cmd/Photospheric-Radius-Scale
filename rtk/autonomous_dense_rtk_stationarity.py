...
def make_ini(model,p,tag):
    ...
    with Path(path).open('w') as f:
        f.write(text);f.write('\n# autonomous matched-ultra+dense RTK 7D Hessian\n')
...
def ev(y,label):
    ...
    print('AUTO_DENSE_RTK_HESSIAN_POINT',json.dumps(rr,sort_keys=True),flush=True)
    return rr
...
rne=ev(np.clip(de,-1,1),'newton_trust_eff')
rnk=ev(np.clip(dk,-1,1),'newton_trust_k01')
best_eff=min(E.values(),key=lambda r:r['score_eff']);best_k01=min(E.values(),key=lambda r:r['score_k01'])
summary={'stage':'autonomous-dense-rtk-7d-stationarity','objective':STATE['objective']['name'],'center':CENTER,'base_steps':dict(BASE),'points':len(E),
'eff':{'S_center':r0['score_eff'],'gradient_y':ge.tolist(),'max_abs_gradient_y':float(np.max(np.abs(ge))),'hessian_y':He.tolist(),'eigenvalues_y':ee.tolist(),'positive_definite':bool(np.all(ee>1e-8)),'newton_delta':de.tolist(),'S_newton':rne['score_eff'],'newton_params':rne['params'],'best_exact_S':best_eff['score_eff'],'best_improvement':r0['score_eff']-best_eff['score_eff'],'best_label':best_eff['label'],'best_params':best_eff['params']},
'k01':{'S_center':r0['score_k01'],'gradient_y':gk.tolist(),'max_abs_gradient_y':float(np.max(np.abs(gk))),'hessian_y':Hk.tolist(),'eigenvalues_y':ek.tolist(),'positive_definite':bool(np.all(ek>1e-8)),'newton_delta':dk.tolist(),'S_newton':rnk['score_k01'],'newton_params':rnk['params'],'best_exact_S':best_k01['score_k01'],'best_improvement':r0['score_k01']-best_k01['score_k01'],'best_label':best_k01['label'],'best_params':best_k01['params']},
'warning':'Local mapping-specific numerical Hessian on frozen production objective; not posterior evidence or global model selection.'}

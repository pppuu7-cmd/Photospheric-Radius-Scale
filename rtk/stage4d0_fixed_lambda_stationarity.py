...
# The steps equal the 0.05 normalized independent poll used by Stage 4D1.
COORDS=[('h',CENTER['h'],0.00035),('Ob',CENTER['Ob'],0.00007),
...
def ev(y,label):
    ...
    row={'label':label,'lambda_D':LAM,'mapping':MAPPING,'S':score(rr)}
    row.update(p)
...
summary={
 ...
 'lambda_D':LAM,'mapping':MAPPING,'center':CENTER,'S_center':S0,'expected_S':EXPECT,
 'coordinates':[{'name':n,'step':s} for n,_,s in COORDS],

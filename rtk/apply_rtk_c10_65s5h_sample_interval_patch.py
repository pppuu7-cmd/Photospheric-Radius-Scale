#!/usr/bin/env python3
"""C10.65s5h endpoint selector only; no physics/tolerance mutation."""
...
t=json.loads((repo/'research/theory_targets/RTK_C10_65S5H_NEXT_K_MULTIBRANCH_LONGER_TIME_TRAJECTORY_TARGET_v1.json').read_text())
assert t['status']=='FROZEN_BEFORE_IMPLEMENTATION'
allowed=[float(x) for x in t['sample_elapsed_tau_Mpc']]
if not any(a.width==x for x in allowed): raise SystemExit('width not in frozen C10.65s5h sample list')
...
p.write_text(s)
print('C10_65S5H_FROZEN_SAMPLE_INTERVAL_PATCH_APPLIED',format(a.width,'.17e'),'old',old[0])
